...
const getAll = async () => {
    const query = "SELECT c.id, c.id_usuario,c.id_perfil,c.comentario_usuario, u.nome AS nome_usuario, u.img_perfil AS url_foto_perfil FROM COMENTARIOS c INNER JOIN USUARIOS u ON c.id_usuario = u.id";
...
const getComentarioNoPerfil = async (id_perfil) => {
    const query = `SELECT c.id, c.id_usuario, c.id_perfil, c.comentario_usuario, u.img_perfil, u.nome 
               FROM COMENTARIOS c 
